...
def collect_long_range_hunting_sights():
    raw_content = fetch_page_content(LONG_RANGE_HUNT_URL)
    ...
    if not long_hunt_rows:
        print("No red dots found")
        return

    headers = BASIC_CSV_HEADERS + ["fov", "elevation"]
    save_path = SAGE_RAT_OUTPUT_DIR / "long_range_hunting.csv"
    save_to_csv(save_path, headers, long_hunt_rows)


if __name__ == '__main__':
    collect_long_range_hunting_sights()