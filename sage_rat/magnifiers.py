from sage_rat.fetch import fetch_page_content
from sage_rat.config import (
    MAGNIF_URL,
    fov_pattern,
    SAGE_RAT_OUTPUT_DIR,
    BASIC_CSV_HEADERS
)
from sage_rat.util import (
    collect_height_and_weight,
    collect_name,
    collect_raw_description,
    save_to_csv,
    clean_raw_desc
)

def collect_magnifiers():
    raw_content = fetch_page_content(MAGNIF_URL)
    content_rows = raw_content.find_all('td', class_='list-td')
    print(f"Found {len(content_rows)} rows")

    mag_rows = []
    for row in content_rows:
        name = collect_name(row)
        raw_desc = collect_raw_description(row)
        height, weight = collect_height_and_weight(raw_desc)

        matches = fov_pattern.findall(raw_desc or "")
        fov = matches[0] if matches else "N/A"

        mag_rows.append({
            "name": name,
            "height": height,
            "weight": weight,
            "raw_desc": clean_raw_desc(raw_desc),
            "fov": fov
        })

    if not mag_rows:
        print("No magnifiers found")
        return

    headers = BASIC_CSV_HEADERS + ["fov"]
    save_path = SAGE_RAT_OUTPUT_DIR / "magnifiers.csv"
    print(f"Writing content to CSV...")
    save_to_csv(save_path, headers, mag_rows)


if __name__ == '__main__':
    collect_magnifiers()